"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RentersService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const renter_entity_1 = require("./entities/renter.entity");
let RentersService = class RentersService {
    renterRepository;
    constructor(renterRepository) {
        this.renterRepository = renterRepository;
    }
    async create(createRenterDto) {
        const renter = this.renterRepository.create(createRenterDto);
        return await this.renterRepository.save(renter);
    }
    async findAll() {
        return await this.renterRepository.find();
    }
    async findOne(id) {
        const renter = await this.renterRepository.findOne({ where: { id } });
        if (!renter) {
            throw new common_1.NotFoundException(`Арендатор с ID ${id} не найден`);
        }
        return renter;
    }
    async findIncomplete() {
        const renters = await this.renterRepository.find({
            select: ['id', 'fullName', 'phone'],
        });
        return renters.map(renter => ({
            id: renter.id,
            fullName: renter.fullName,
            phone: renter.phone,
        }));
    }
    async update(id, updateRenterDto) {
        await this.renterRepository.update(id, updateRenterDto);
        return this.findOne(id);
    }
    async remove(id) {
        const result = await this.renterRepository.delete(id);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Арендатор с ID ${id} не найден`);
        }
    }
};
exports.RentersService = RentersService;
exports.RentersService = RentersService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(renter_entity_1.Renter)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], RentersService);
//# sourceMappingURL=renters.service.js.map